...
def is_repeated_twice(id: int) -> bool:
    """check if product ID is made of a sequence repeated twice"""
    id_str = str(id)
    if len(id_str) % 2 != 0:
        return False
    mid = len(id_str) // 2
...
def sum_invalid_ids_with_smudge(start_id: int, end_id: int) -> int:
    """sum all product indices matching smudge pattern (progressive prefix removal)"""
    sum = 0
    for id in range(start_id, end_id + 1):
        id_str = str(id)
        prefix = ""
